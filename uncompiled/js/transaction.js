 ...
 * @constructor
 */
elevate300.Transaction = function(casinoOptionsStr, selectCasino, addOnsUsedStr) {

  /**
   * List of casinos for this trip.
   * @type {Array.<elevate300.Casino>}
   */
  this.casinoOptions = [];
  if (casinoOptionsStr.length != 0) {
    casinoOptionsStr = casinoOptionsStr.replace(/&\#39;/g, "'");
    this.casinoOptions = /** @type {Array.<elevate300.Casino>} */
        (goog.json.unsafeParse(casinoOptionsStr));    
  }
...
elevate300.Transaction.prototype.locationSuccessHandler_ = function(pos) {
  ...
  // Temporarially list all the trip specific casinos
  var casinoNames = [];
  for (var i = 0; i < this.casinoOptions.length; i++) {
    casinoNames.push(this.casinoOptions[i].name);
  }
  this.updateCasinoOptions_(casinoNames);
//  for (var i = 0; i < this.casinoOptions.length; i++) {
//    var casino = this.casinoOptions[i];
//    this.logger.info("Casino name = " + casino.name);
//    this.logger.info("Latitude = " + casino.latitude);
//    this.logger.info("Longitude = " + casino.longitude);
//  }
...
elevate300.Transaction.prototype.updateAddOnVisibility_ = function() {
  ...
  this.logger.info("updateAddOnVisibility_ was called");
...
elevate300.Transaction.prototype.resizeElements_ = function() {
  ...
  var casinoSelect = goog.dom.getElement('casino-select');
  goog.style.setStyle(casinoSelect, 'width', textBoxWidth);
